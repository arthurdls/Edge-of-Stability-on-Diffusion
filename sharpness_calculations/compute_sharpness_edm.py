...
def infer_model_architecture(checkpoint_path: Path) -> dict:
    """
    Infer model architecture parameters from an EDM checkpoint file.
    
    EDM checkpoints contain EDMPrecond wrapper, so keys are like 'model.conv_in.weight'
    instead of 'conv_in.weight'.
    ...
    # EDM checkpoints have EDMPrecond wrapper, so keys are like 'model.conv_in.weight'
    # Try to find conv_in layer with different possible key names
    conv_in_key = None
    for possible_key in ['model.conv_in.weight', 'conv_in.weight', 'net.conv_in.weight']:
    ...
    # Try to find time_mlp layer (EDM uses model.time_mlp.0.weight)
    time_mlp_key = None
    for possible_key in ['model.time_mlp.0.weight', 'time_mlp.0.weight', 'net.time_mlp.0.weight']:
        if possible_key in state:
...
def compute_lambda_max_for_checkpoints(
    ...
    sigma_samples: List[float] = None,  # List of sigma (noise level) values to compute lambda max for
    ...
    """
    Compute lambda max for all EDM checkpoints in a directory.
    ...
    Args:
        ...
        sigma_samples: List of sigma (noise level) values to compute lambda max for (default: [1.0])
    ...
    Returns:
        Dictionary mapping sigma_sample -> (epochs, lambda_max_values) tuples
    """
    if sigma_samples is None:
        sigma_samples = [1.0]  # Default sigma value
    ...
    # Initialize model (EDMPrecond wrapper around UNet)
    # Architecture only, weights will be loaded from checkpoint
    ...
    model = EDMPrecond(unet)  # Wrap UNet with EDMPrecond
    ...
    # Validate sigma_samples are positive
    for sigma in sigma_samples:
        if sigma <= 0:
            raise ValueError(f"sigma_sample={sigma} must be positive")
    
    # Store results for each sigma_sample
    results = {sigma: {'epochs': [], 'lambda_max_values': []} for sigma in sigma_samples}
    ...
    for checkpoint_path, epoch in tqdm(checkpoint_files, desc="Computing lambda max"):
        try:
            ...
            # Compute lambda max for each sigma_sample
            for sigma_sample in sigma_samples:
                try:
                    ...
                    # Compute lambda max for this checkpoint and sigma value
                    ...
                    results[sigma_sample]['epochs'].append(epoch)
                    results[sigma_sample]['lambda_max_values'].append(lambda_max)
                    
                    if verbose:
                        print(f"  sigma={sigma_sample}: Lambda max: {lambda_max:.6f}")
                        
                except Exception as e:
                    print(f"  Error processing sigma_sample={sigma_sample} for checkpoint {checkpoint_path.name}: {e}")
                    if verbose:
                        import traceback
                        traceback.print_exc()
                    continue
    ...
    # Convert to format: {sigma_sample: (epochs, lambda_max_values)}
    return {sigma: (results[sigma]['epochs'], results[sigma]['lambda_max_values']) for sigma in sigma_samples}
...
def plot_lambda_max_history(
    ...
    """
    Plot lambda max values over epochs for multiple sigma values.
    
    Args:
        results: Dictionary mapping sigma_sample -> (epochs, lambda_max_values) tuples
    ...
    # Plot each sigma value as a separate line
    colors = plt.cm.tab10(np.linspace(0, 1, len(results)))
    for i, (sigma_sample, (epochs, lambda_max_values)) in enumerate(sorted(results.items())):
        plt.plot(epochs, lambda_max_values, 'o-', linewidth=2, markersize=5, 
                label=f'σ={sigma_sample}', color=colors[i])
...
def main():
    ...
    # EDM uses continuous sigma (noise levels) instead of discrete timesteps
    # Typical sigma range: [0.002, 80] (from EDM paper)
    # Sample sigma values across the range (log scale)
    sigma_samples = [0.01, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 50.0, 80.0]
    max_iterations = 100
    reltol = 1e-2
    ...
    print(f"Sigma samples: {sigma_samples}")
    ...
    # Compute lambda max for all checkpoints and sigma values
    ...
    # Print summary for each sigma value
    print("\n" + "=" * 60)
    print("Summary")
    print("=" * 60)
    for sigma_sample in sorted(results.keys()):
        epochs, lambda_max_values = results[sigma_sample]
        if len(epochs) > 0:
            print(f"\nσ={sigma_sample}:")
    ...
    # Plot results (all sigma values on same plot)
