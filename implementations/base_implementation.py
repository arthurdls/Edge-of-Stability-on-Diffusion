...
class UNet(nn.Module):
    ...
    """
    def __init__(self, in_ch=1, base_ch=32, time_emb_dim=32):
        super().__init__()
...
def train_ddim(model, schedule, train_loader, device, epochs=100, lr=2e-4, save_dir='./runs', measure_bs=16, checkpoint=None):
    """
    ...
    if checkpoint is not None:
        ...
        # Load map_location ensures we don't try to load to GPU 0 if using a different device
        ckpt_data = torch.load(checkpoint, map_location=device, weights_only=True) 
        
        # Load model weights
        # Note: We load into 'model', not 'compiled_model', to avoid prefix issues
