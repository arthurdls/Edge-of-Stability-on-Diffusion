...
def load_sampler_from_checkpoint(
    ...
    """
    Load TimestepSampler from checkpoint if available.
    
    Args:
        checkpoint_path: Path to the checkpoint file
        device: Device to load the sampler on
        
    Returns:
        TimestepSampler instance if found, None otherwise
    """
    checkpoint_path = Path(checkpoint_path)
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"Checkpoint not found: {checkpoint_path}")
    ...
    # Check if sampler state exists
    if 'sampler_state' not in checkpoint:
        return None
    
    # Import TimestepSampler
    try:
        from ..adaptive_sampling import TimestepSampler
    except ImportError:
        try:
            from implementations.adaptive_sampling import TimestepSampler
        except ImportError:
            # Fallback: define TimestepSampler locally
    ...
    sampler = TimestepSampler(in_channels=3).to(device)
    ...
    for key, value in sampler_state.items():
        if key.startswith('_orig_mod.'):
            new_sampler_state[key[len('_orig_mod.'):]] = value
        else:
            new_sampler_state[key] = value
    
    sampler.load_state_dict(new_sampler_state, strict=False)
    sampler.eval()
    
    return sampler


def adaptive_p_losses(model, schedule, sampler, x_start, t_target=None, device=None, use_mean=True):
    """
    Compute loss using adaptive timestep sampling.
    
    This function uses the TimestepSampler to generate timesteps from a Beta distribution
    based on the input x_start, then computes the standard p_losses.
    
    Args:
        model: The diffusion model (UNet)
        schedule: DiffusionSchedule instance
        sampler: TimestepSampler instance
        x_start: Input data tensor (batch_size, channels, height, width)
        t_target: Target timestep(s) to use. If provided, uses these exact timesteps
                  instead of sampling from the Beta distribution. If None, uses the sampler.
        device: Device to run computation on
        use_mean: If True and t_target is None, use the mean of the Beta distribution (deterministic).
                  If False and t_target is None, sample from the distribution (stochastic). Default: True.
    ...
    # If target timestep is provided, use it directly
    if t_target is not None:
        # Convert to tensor if needed
        if isinstance(t_target, (int, float)):
            batch_size = x_start.shape[0]
            t = torch.full((batch_size,), int(t_target), dtype=torch.long, device=device)
        else:
            t = t_target.to(device) if isinstance(t_target, torch.Tensor) else t_target
    else:
        # Get Beta distribution from sampler
        dist = sampler.get_dist(x_start)
        
        # Get timesteps from Beta distribution
        if use_mean:
            # Use mean for deterministic behavior: mean = alpha / (alpha + beta)
            # This gives consistent results for Hessian computation
            alpha, beta = sampler.forward(x_start)
            t_float = alpha / (alpha + beta)  # Mean of Beta distribution
        else:
            # Sample timesteps from Beta distribution (values in [0, 1])
            t_float = dist.sample()
        
        # Map [0, 1] to integer timesteps [0, T-1]
        t = (t_float * (schedule.timesteps - 1)).long().to(device)
    
    # Compute standard p_losses with adaptive timesteps
    try:
        from ..base_implementation import p_losses
    except ImportError:
        from implementations.base_implementation import p_losses
    
    return p_losses(model, schedule, x_start, t)
...
def compute_lambda_max_from_checkpoint_simple(
    ...
    for diffusion models using the p_losses function. Supports adaptive sampling when
    use_adaptive_sampling=True and sampler is available in checkpoint.
    ...
    Args:
        ...
        t_sample: Timestep tensor (batch_size,). If None and not using adaptive sampling,
                  random timesteps are generated. Ignored if use_adaptive_sampling=True.
        device: Device to run computation on
        ...
        return_sym_evecs (bool, optional): If True and P is provided, returns eigenvectors of
                                           P^{-1/2} H P^{-1/2}. If False, returns right eigenvectors
                                           of P^{-1} H. Defaults to False.
        use_adaptive_sampling (bool): If True, use adaptive timestep sampling with TimestepSampler.
                                     Defaults to True for this adaptive sampling version.
    ...
    Example:
        ...
        ...     model, schedule, x, P=P, use_adaptive_sampling=True
        ... )
    """
    # Move to device
    if device is None:
    ...
    # Load sampler if using adaptive sampling
    sampler = None
    if use_adaptive_sampling:
        sampler = load_sampler_from_checkpoint(checkpoint_path, device=device)
        if sampler is None:
            print(f"  ⚠️  Warning: No sampler_state found in checkpoint. Falling back to standard p_losses.")
            use_adaptive_sampling = False
        else:
            print(f"  Loaded TimestepSampler from checkpoint for adaptive sampling")
    ...
    # Create loss function based on whether we're using adaptive sampling
    if use_adaptive_sampling and sampler is not None:
        # Use adaptive sampling loss function
        # If t_sample is provided, use it as target timestep; otherwise use sampler's distribution
        # The loss function receives (model, schedule, x_sample, ...) when unpacked with *data_sample
        # We need to capture t_sample in the closure
        target_t = t_sample
        def loss_fn(m, s, x, *args):
            # If t_sample was passed as an argument (from data_sample tuple), use it
            # Otherwise use the captured target_t
            if len(args) > 0 and args[0] is not None:
                t_target = args[0]
            else:
                t_target = target_t
            # Use t_target as target timestep if provided, otherwise let sampler choose
            return adaptive_p_losses(m, s, sampler, x, t_target=t_target, device=device, use_mean=True)
        # For adaptive sampling, we include t_sample in data_sample if provided
        # This allows us to compute lambda max at specific timesteps
        if t_sample is not None:
            # Convert t_sample to proper format if needed
            if isinstance(t_sample, (int, float)):
                batch_size = x_sample.shape[0]
                t_sample_tensor = torch.full((batch_size,), int(t_sample), dtype=torch.long, device=device)
            else:
                t_sample_tensor = t_sample.to(device) if isinstance(t_sample, torch.Tensor) else t_sample
            data_sample = (schedule, x_sample, t_sample_tensor)
        else:
            data_sample = (schedule, x_sample)
    else:
        # Use standard p_losses
        try:
            from ..base_implementation import p_losses
        except ImportError:
            from implementations.base_implementation import p_losses
        ...
        if t_sample is None:
            batch_size = x_sample.shape[0]
            t_sample = torch.randint(0, schedule.timesteps, (batch_size,), device=device)
            ...
            t_sample = torch.full((batch_size,), int(t_sample), dtype=torch.long, device=device)
        
        t_sample = t_sample.to(device)
        def loss_fn(m, s, x, t):
            return p_losses(m, s, x, t)
        data_sample = (schedule, x_sample, t_sample)
