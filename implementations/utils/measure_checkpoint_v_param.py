...
def compute_lambda_max_from_checkpoint_simple(
    ...
    for diffusion models using the v-parameterization loss function (v_param_loss).
    ...
    Example:
        ...
        ... )
    """
    # Import v_param_loss here to avoid circular imports
    try:
        from ..v_parametrization import v_param_loss
    except ImportError:
        # Fallback: define v_param_loss locally if import fails
    ...
    return compute_lambda_max_from_checkpoint(
        checkpoint_path=checkpoint_path,
        model=model,
        loss_fn=lambda m, s, x, t: v_param_loss(m, s, x, t),
        data_sample=(schedule, x_sample, t_sample),
        device=device,
        use_ema=use_ema,
        k=k,
