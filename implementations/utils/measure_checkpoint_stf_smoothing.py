...
def compute_lambda_max_from_checkpoint(
    ...
    Example:
        >>> from implementations.stf_smoothing import UNet, DiffusionSchedule, stf_loss
        >>> from implementations.utils.measure_checkpoint_stf_smoothing import (
        ...
        >>> 
        >>> # Compute lambda max of preconditioned Hessian using STF loss
        >>> lambda_max = compute_lambda_max_from_checkpoint(
        ...     checkpoint_path='./runs/checkpoint_epoch_10.pt',
        ...     model=model,
        ...     loss_fn=lambda m, s, x, t: stf_loss(m, s, x, t),
...
def compute_lambda_max_from_checkpoint_simple(
    ...
    for diffusion models using the STF (Stable Target Field) smoothing loss function.
    ...
    Example:
        >>> from implementations.stf_smoothing import UNet, DiffusionSchedule
        >>> from implementations.utils.measure_checkpoint_stf_smoothing import (
        ...     create_preconditioner_from_checkpoint, create_constant_preconditioner
        ... )
        >>> import torch
        ...
        ... )
    """
    # Import stf_loss here to avoid circular imports
    try:
        from ..stf_smoothing import stf_loss
    except ImportError:
        # Fallback: define stf_loss locally if import fails
        # This is a simplified version - should match stf_smoothing.py
        def stf_loss(model, schedule, x_start, t):
            # STF loss implementation
            noise = torch.randn_like(x_start)
            x_noisy = schedule.q_sample(x_start=x_start, t=t, noise=noise)
            
            # Prepare for pairwise distance calculation
            B = x_start.shape[0]
            ...
            one_minus_alpha_bar = (1. - schedule.alphas_cumprod[t]).view(-1, 1)
            
            # Calculate squared Euclidean distance matrix
            # Force float32 for distance calculation to prevent overflow/underflow
            flat_noisy_f32 = flat_noisy.float()
            flat_start_f32 = flat_start.float()
            
            norm_noisy_sq = (flat_noisy_f32**2).sum(dim=1, keepdim=True)
            norm_start_sq = (flat_start_f32**2).sum(dim=1).unsqueeze(0)
            dot_prod = torch.mm(flat_noisy_f32, flat_start_f32.t())
            ...
            # Construct stable target
            weights = weights.to(x_start.dtype)
