...
def load_checkpoint(checkpoint_path: Union[str, Path], 
    ...
    # For EDM models, attention layers are inside model.model (EDMPrecond wraps UNet)
    attention_layer_names = ['down2_attn', 'mid_attn', 'up1_attn']
    
    # Check if model is EDMPrecond wrapper (has 'model' attribute)
    if hasattr(model, 'model') and hasattr(model.model, 'down2_attn'):
        # EDM structure: model.model contains the UNet
        for attn_name in attention_layer_names:
            if hasattr(model.model, attn_name):
                setattr(model.model, attn_name, nn.Identity())
                print(f"  Replaced model.{attn_name} with Identity (EDM structure, avoids Hessian computation issues)")
    else:
        # Direct UNet structure (for backward compatibility)
        for attn_name in attention_layer_names:
...
def compute_lambda_max_from_checkpoint_simple(
    ...
    sigma_sample: Optional[Union[float, torch.Tensor]] = None,
    device: Optional[torch.device] = None,
    use_ema: bool = False,
    k: int = 1,
    max_iterations: int = 100,
    ...
    """
    Simplified version for EDM models that automatically handles the loss function.
    
    This is a convenience wrapper around compute_lambda_max_from_checkpoint specifically
    for EDM (Elucidating the Design Space of Diffusion-Based Generative Models) models
    using the edm_loss function.
    
    Args:
        checkpoint_path: Path to the checkpoint file
        model: The model instance (EDMPrecond wrapper around UNet)
        x_sample: Input data tensor (batch_size, channels, height, width)
        sigma_sample: Sigma (noise level) value(s) to compute lambda max for.
                     Can be a float, tensor, or None. If None, uses default sigma=1.0.
                     If tensor, should have shape (batch_size,).
        ...
        return_sym_evecs (bool, optional): If True and P is provided, returns eigenvectors of
                                           P^{-1/2} H P^{-1/2}. If False, returns right eigenvectors
                                           of P^{-1} H. Defaults to False.
        use_preconditioner: If True, create preconditioner from checkpoint
        optimizer_type: Type of optimizer ('adam', 'rmsprop', 'sgd', or 'auto')
        learning_rate: Learning rate for constant preconditioner
        sigma_data: EDM sigma_data parameter (default: 0.5)
    ...
    Example:
        ...
        >>> from implementations.edm_style_preconditioning import EDMPrecond
        >>> from implementations.utils.measure_checkpoint_edm import (
        ...     create_preconditioner_from_checkpoint, create_constant_preconditioner
        ... )
        >>> import torch
        >>> 
        >>> unet = UNet()
        >>> model = EDMPrecond(unet)
        ...
        ...     model, x, sigma_sample=1.0, P=P
        ... )
    """
    # Import edm_loss here to avoid circular imports
    try:
        from ..edm_style_preconditioning import edm_loss
    except ImportError:
        # Fallback: define edm_loss locally if import fails
        def edm_loss(model, x_start, sigma, sigma_data=0.5):
            # Fixed sigma version for lambda max computation
            batch_size = x_start.shape[0]
            if isinstance(sigma, (int, float)):
                sigma = torch.full((batch_size, 1, 1, 1), float(sigma), device=x_start.device, dtype=x_start.dtype)
            elif sigma.dim() == 0:
                sigma = sigma.reshape(1, 1, 1, 1).expand(batch_size, -1, -1, -1)
            elif sigma.dim() == 1:
                sigma = sigma.view(-1, 1, 1, 1)
            
            # Calculate loss weight lambda(sigma)
            ...
            # Run the preconditioned model D_theta
            D_x = model(x_noisy, sigma.flatten())
            
            # Weighted MSE against the CLEAN image (x_start)
            loss = weight * ((D_x - x_start) ** 2)
            return loss.mean()
    
    # Generate sigma if not provided (default to 1.0)
    if sigma_sample is None:
        sigma_sample = 1.0
    
    # Convert sigma to tensor format
    batch_size = x_sample.shape[0]
    if isinstance(sigma_sample, (int, float)):
        sigma_tensor = torch.full((batch_size,), float(sigma_sample), device=x_sample.device if device is None else device, dtype=torch.float32)
    elif isinstance(sigma_sample, torch.Tensor):
        if sigma_sample.dim() == 0:
            sigma_tensor = sigma_sample.expand(batch_size).to(x_sample.device if device is None else device)
        elif sigma_sample.dim() == 1:
            if sigma_sample.shape[0] == 1:
                sigma_tensor = sigma_sample.expand(batch_size).to(x_sample.device if device is None else device)
            elif sigma_sample.shape[0] == batch_size:
                sigma_tensor = sigma_sample.to(x_sample.device if device is None else device)
            else:
                raise ValueError(f"sigma_sample shape {sigma_sample.shape} doesn't match batch_size {batch_size}")
        else:
            raise ValueError(f"sigma_sample must be scalar or 1D tensor, got shape {sigma_sample.shape}")
    else:
        raise ValueError(f"sigma_sample must be float, int, or tensor, got {type(sigma_sample)}")
    ...
    # Create loss function with fixed sigma
    def edm_loss_fixed_sigma(model, x_start, sigma_vals, sigma_data=sigma_data):
        """EDM loss with fixed sigma values (not sampled from distribution)"""
        batch_size = x_start.shape[0]
        # Convert sigma to proper shape for EDM
        if sigma_vals.dim() == 1:
            sigma = sigma_vals.view(-1, 1, 1, 1)
        else:
            sigma = sigma_vals
    ...
    return compute_lambda_max_from_checkpoint(
        checkpoint_path=checkpoint_path,
        model=model,
        loss_fn=lambda m, x, sigma: edm_loss_fixed_sigma(m, x, sigma, sigma_data=sigma_data),
        data_sample=(x_sample, sigma_tensor),
        device=device,
        use_ema=use_ema,
        k=k,
        max_iterations=max_iterations,
